...
- Error visibility is live end-to-end. `src/lib/errors.ts` centralizes runtime reporting with `report('error' | 'warn' | 'info', ...)`, FIFO retention, subscribers, console mirroring, and one-time global `error` / `unhandledrejection` handlers. `src/components/system/ErrorConsole.tsx` renders the in-app surface: unread-error banner, bottom status strip, expandable event log, and clear/read controls. Activated at startup via `installGlobalErrorHandlers(window)` in `src/main.tsx`.
